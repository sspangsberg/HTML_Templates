...
foreach ($navItems as $ni) {
    ...
    */
    ...
    $ni->li_classes = implode(" ", $li_classes);
    $ni->a_classes = implode(" ", $a_classes);
}
...
if (count($navItems) > 0) {
    ...
    foreach ($navItems as $ni) {
        ...
        echo '<li class="' . $ni->li_classes . '">'; //opens a nav item
        $name = (isset($translate) && $translate == true) ? t($ni->name) : $ni->name;
        $data_attr = ( $ni->hasSubmenu && $ni->level == 1 ) ? 'data-toggle="dropdown"' : '';
        $caret = ( $ni->hasSubmenu && $ni->level == 1 ) ? ' <b class="caret"></b>' : '';
        echo '<a href="' . $ni->url . '" target="' . $ni->target . '" class="' . $ni->a_classes . '" '. $data_attr .' >' . $name . $caret . '</a>';
